...
class QRForge {
    ...
    initCustomizationControls() {
        const inputs = document.querySelectorAll('.custom-control');
        inputs.forEach(input => {
    ...
    async updateCustomPreview() {
        ...
        const settings = {
            ...
            width: 300,
            height: 300,
            colorDark: document.getElementById('custom-fg')?.value || "#000000",
            colorLight: document.getElementById('custom-bg')?.value || "#ffffff",
            correctLevel: 'H'
        };
        ...
        // Generate QR using qrcode-generator
        const typeNumber = 0;
        const qr = qrcode(typeNumber, settings.correctLevel);
        qr.addData(settings.text);
        qr.make();

        const moduleCount = qr.getModuleCount();
        const pixelSize = Math.max(2, Math.floor(settings.width / moduleCount));
        const margin = 4;
        const canvasSize = (moduleCount + 2 * margin) * pixelSize;
        ...
        // Add logo if present (not implemented in this simplified version but placeholder logic kept)
        // ...
